...
When using `yvm exec`, the appropriate `yarn` version is executed using the `node` available in the current context. If you are using `nvm`, you can avoid having to execute `nvm use` before a `yvm exec` by enabling the nvm integration like so:

```sh
YVM_NVM_INTEGRATION=1 yvm exec my-command
```
